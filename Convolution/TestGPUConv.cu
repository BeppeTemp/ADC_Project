...
__global__ void ConvolutionKernel(float *mat_start, float *mat_res, const float *mask, int size, float *mat_temp)
{
    int tx = threadIdx.x;
    int ty = threadIdx.y;
    ...
    if (ty < TILE_WIDTH && tx < TILE_WIDTH)
    {
        for (int i = 0; i < MASK_SIZE; i++)
        {
            for (int j = 0; j < MASK_SIZE; j++)
            {
                ...
                //printf("mammt a pecrssss %d\n", output);
            }
        }
        if (row_o < size && col_o < size)
        {
            //mat_temp[(row_o * MASK_SIZE) + col_o] = output;
            mat_res[row_o * MASK_SIZE + col_o] = output;
        }
    }
}
...
int main()
{
    int sizes[5] = {4, 4096, 8192, 16384, 32768};

    float *mat_start_host, *mat_res_host, *mask_host;
    float *mat_start_dev, *mat_res_dev, *mask_dev;
    float *mat_temp = (float *)malloc(MASK_SIZE * MASK_SIZE * sizeof(float));
    float *mat_transpose = (float *)malloc(MASK_SIZE * MASK_SIZE * sizeof(float));
    ...
    for (int i = 0; i < MASK_SIZE; i++)
    {
        for (int j = 0; j < MASK_SIZE; j++)
        {
            mat_transpose[j * MASK_SIZE + i] = mask_host[i * MASK_SIZE + j];
        }
    }

    // mask init and upload
    cudaMalloc((void **)&mask_dev, MASK_SIZE * MASK_SIZE * sizeof(float));
    cudaMemcpy(mask_dev, mat_transpose, MASK_SIZE * MASK_SIZE * sizeof(float), cudaMemcpyDefault);
    ...
    for (int k = 0; k < 1; k++)
    {
        ...
        cudaMalloc((void **)&mat_temp, MASK_SIZE * MASK_SIZE* sizeof(float));
        ...
        ConvolutionKernel<<<gridDim, blockDim>>>(mat_start_dev, mat_res_dev, mask_dev, sizes[k], mat_temp);
