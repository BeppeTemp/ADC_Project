...
enum comptuationDevice { dev_both, dev_gpu, dev_cpu };
...
__host__ void multiplyMatrices(squareMatrix a,
                               squareMatrix b,
                               squareMatrix result) {
    ...
    for (int i = 0; i < result.dimension; i++) {
        for (int j = 0; j < result.dimension; j++) {
            ...
            for (int k = 0; k < result.dimension; k++)
                result.elements[j + result.dimension * i] +=
                    a.elements[k + result.dimension * i] *
                    b.elements[j + result.dimension * k];
        }
    }
}

__global__ void multiplyMatricesParallel(int* mat_a,
                                         int* mat_b,
                                         int* mat_results,
                                         int dimension) {
    ...
    if (idx < dimension * dimension) {
        ...
        for (int i = 0; i < dimension; i++) {
            ...
            mat_results[idx] += mat_a[i + dimension * row] *
                                mat_b[idx % dimension + dimension * i];
        }
    }
}
...
__host__ void saveMatrixToFile(squareMatrix mat_sav, char* label) {
    ...
    snprintf(fileNameBuffer, sizeof fileNameBuffer, "%s/%sx%s_X_%sx%s_%s.txt",
             startup.outputDirectory, dim, dim, dim, dim, label);

    FILE* fp = fopen(fileNameBuffer, "w");
    if (fp == nullptr)
        printf("Could not log to file\n");
    else {
        for (int i = 0; i < mat_sav.dimension * mat_sav.dimension; i++) {
            if (i % mat_sav.dimension == 0 && i != 0)
                fprintf(fp, "\n");
...
__host__ void testHostPreformance(squareMatrix mat_a, squareMatrix mat_b) {
    ...
    squareMatrix mat_results = {
        (int*)malloc(sizeof(int) * mat_a.dimension * mat_a.dimension),
        mat_a.dimension};
...
__host__ void testDevicePreformance(squareMatrix mat_a, squareMatrix mat_b) {
    ...
    if (mat_a.dimension != mat_b.dimension)
        exit(1);
    ...
    before = clock();
    gpuErrchk(cudaMemcpy(dev_mat_a, mat_a.elements, allocationsize,
                         cudaMemcpyHostToDevice));
    gpuErrchk(cudaMemcpy(dev_mat_b, mat_b.elements, allocationsize,
                         cudaMemcpyHostToDevice));
    printTime(clock() - before);
    ...
    multiplyMatricesParallel<<<totalThreadsNeeded / startup.threadsPerBlock + 1,
                               startup.threadsPerBlock>>>(
        dev_mat_a, dev_mat_b, dev_mat_results, mat_a.dimension);
    gpuErrchk(cudaGetLastError());
    gpuErrchk(cudaMemcpy(mat_results.elements, dev_mat_results, allocationsize,
...
__host__ void testMatrixMultiplicationPreformance(int dimension) {
    ...
    if (startup.device != dev_cpu)
        testDevicePreformance(mat_a, mat_b);
    if (startup.device != dev_gpu)
...
int main(int argc, char** argv) {
    for (int i = 0; i < argc; i++) {
        ...
        if (strcmp(argv[i], "--device") == 0 && i + 1 < argc)
            if (strcmp(argv[i + 1], "gpu") == 0)
                startup.device = dev_gpu;
            else if (strcmp(argv[i + 1], "cpu") == 0)
                startup.device = dev_cpu;
            else if (strcmp(argv[i + 1], "both") == 0)
                startup.device = dev_both;
        if (strcmp(argv[i], "--random_mod") == 0 && i + 1 < argc)
            startup.randomMod = atoi(argv[i + 1]);
        if (strcmp(argv[i], "--max_dimension") == 0 && i + 1 < argc)
            startup.maxDimension = atoi(argv[i + 1]);
        if (strcmp(argv[i], "--seed") == 0 && i + 1 < argc)
            startup.seedValue = atoi(argv[i + 1]);
        if (strcmp(argv[i], "--start_dimension") == 0 && i + 1 < argc)
            startup.startDimension = atoi(argv[i + 1]);
        if (strcmp(argv[i], "--only") == 0 && i + 1 < argc)
            startup.onlyMatrixSize = atoi(argv[i + 1]);
        if (strcmp(argv[i], "--block_threads") == 0 && i + 1 < argc)
            startup.threadsPerBlock = atoi(argv[i + 1]);
        if (strcmp(argv[i], "--mat_print") == 0)
            startup.matPrint = true;
        if (strcmp(argv[i], "--mat_save") == 0) {
            startup.matSave = true;
            if (i + 1 < argc && strstr(argv[i + 1], "--") == NULL)
                startup.outputDirectory = argv[i + 1];
        }
    }
    ...
    /*Otherwise, double matrix size until vram is completely filled*/
        ...
        unsigned int maxMatrixDimension =
            calculateLargestPossibleMatrixDimension();
        for (int i = startup.startDimension;
             i != maxMatrixDimension * 2 && i <= startup.maxDimension; i *= 2) {
